...
protected:
	ComPtr<ID3D11RasterizerState> createRSState(RasterizerPreset preset);
	ComPtr<ID3D11RasterizerState> createRSState(
	...
	ComPtr<ID3D11DepthStencilState> createDSState(
		bool depthEnable,
	...
	void unbindRenderTargets(UINT count);
	void unbindShaderResources(UINT startSlot, UINT count);
	void unbindComputeUAVs(UINT startSlot, UINT count);
